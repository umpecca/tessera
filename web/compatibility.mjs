...
export function compatibilityDiagnostics(info) {
  return [
    ...
    `Paint coalescing: ${info.paintCoalescing ? "On" : "Off"}`,
    `Server output coalescing: ${info.outputCoalescing ? "On" : "Off"}`,
    "Renderer row paths:",
    ...(info.rendererRows || ["Not collected"]),
    "Rendering costs (sampled while Settings is open; recent rates use the last 5 seconds):",
    ...(info.renderingCosts || []),
    "Output timing (intervals and delays are p50/p95/max ms; network jitter is transit above the fastest sample):",
    ...(info.outputTiming?.length ? info.outputTiming : ["Not captured"]),
