...
import { defaultWheelSensitivity, normalizeWheelSensitivity, wheelDeltaUnits } from "./wheel-sensitivity.mjs";
...
test("normalizes wheel sensitivity within the supported range", () => {
  ...
  assert.equal(defaultWheelSensitivity, 1.5);
  assert.equal(normalizeWheelSensitivity(0), 1.5);
  assert.equal(normalizeWheelSensitivity(5), 1.5);
  assert.equal(normalizeWheelSensitivity("fast"), 1.5);
});
